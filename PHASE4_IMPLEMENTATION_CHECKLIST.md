...
- The client retains the idempotency key across retry/reload, and the reel API discovers expired reservations and durable `provider_succeeded` results so the UI can resume them safely. It retires only the matching key after success, terminal consumption, or confirmed acknowledgement; a stale acknowledgement cannot clear a newer request. Revision keys are bound to the original script version and instruction, and changed payloads receive a new identity.
