...
export type BeginStageProviderSubmissionResult =
  | { disposition: "started"; providerAttemptId: string }
  | { disposition: "uncertain" };

/**
 * Persists a provider-submission marker before an external request starts.
 * A superseding stage attempt must observe an unresolved marker and stop
 * rather than repeat a request whose outcome is not known locally.
 */
export async function beginStageProviderSubmission(input: {
  pipelineId: string;
  userId: string;
  stageAttemptId: string;
  leaseOwner: string;
  stage: string;
}): Promise<BeginStageProviderSubmissionResult> {
  return sequelize.transaction(async (transaction) => {
    await requireOwnedPipeline(input.pipelineId, input.userId, transaction, true);
    ...
    if (
      !checkpoint
      || checkpoint.pipelineId !== input.pipelineId
      || checkpoint.stage !== input.stage
      || !hasActiveLease(checkpoint, input.leaseOwner)
    ) throw leaseLost();

    if (checkpoint.providerAttemptId) return { disposition: "uncertain" };
    const unresolved = await WorkflowStageAttempt.findOne({
      where: {
        pipelineId: input.pipelineId,
        workflowVersion: checkpoint.workflowVersion,
        stage: input.stage,
        providerAttemptId: { [Op.ne]: null },
        id: { [Op.ne]: checkpoint.id },
      },
      transaction,
      lock: transaction.LOCK.UPDATE,
    });
    if (unresolved) return { disposition: "uncertain" };

    const providerAttemptId = randomUUID();
    checkpoint.providerAttemptId = providerAttemptId;
    await checkpoint.save({ transaction });
    return { disposition: "started", providerAttemptId };
  });
}

/** Clear only when the provider explicitly rejected the request before start. */
export async function clearStageProviderSubmission(input: {
  stageAttemptId: string;
  leaseOwner: string;
  ...
  return sequelize.transaction(async (transaction) => {
    ...
    if (
      !checkpoint
      || !hasActiveLease(checkpoint, input.leaseOwner)
      || checkpoint.providerAttemptId !== input.providerAttemptId
    ) throw leaseLost();
    checkpoint.providerAttemptId = null;
    await checkpoint.save({ transaction });
  });
}
